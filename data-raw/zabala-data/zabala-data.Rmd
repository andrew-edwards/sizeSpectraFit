...
fung_dat <- cbind(fung_use_simplify,
  ...
  dplyr::filter(!is.na(alpha),  # remove species with no length-weight coefficients
                bin_count != 0,
                weight_bin_min >= 4)    # 4g and above. TODO TODO this should
                                        # work as now corrected biomass_bin_min
                                        # to weight_bin_min.
...
Woo-hoo, matches Table S.2


HERE - check plot, commit, then work out which version to keep (which is more
generalisable), then simplify this. Actually make
