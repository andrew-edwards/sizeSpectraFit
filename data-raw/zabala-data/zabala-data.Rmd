...
# exclude from resnep21_fg trawls done beside the MPA -- too close
...
# separate resnep 2017 and 2021 (MPA & FG),
resnep17 <- subset(resnep, Any != "2021")
resnep21 <- subset(resnep, Any != "2017")
resnep21_mpa <- subset(resnep21, Site == "MPA")
...
# resnep21_mpa
resnep21_mpa_crustacea <- subset(resnep21_mpa, AgrupamentTaxonomic == "Crustacea")
resnep21_mpa_cephalopoda <- subset(resnep21_mpa, AgrupamentTaxonomic == "Cephalopoda")
resnep21_mpa_actinopterygii <- subset(resnep21_mpa, AgrupamentTaxonomic == "Actinopterygii")
resnep21_mpa_chondrichthyes <- subset(resnep21_mpa, AgrupamentTaxonomic == "Chondrichthyes")

resnep21_mpa_cephalopoda_small <- subset(resnep21_mpa_cephalopoda, PesoIndividual < 10)
resnep21_mpa_cephalopoda_big <- subset(resnep21_mpa_cephalopoda, PesoIndividual > 10)
...
usethis::use_data(resnep17_actinopterygii,
                  ...
                  resnep21_mpa_actinopterygii,
                  resnep21_mpa_cephalopoda,
                  resnep21_mpa_cephalopoda_big,
                  resnep21_mpa_cephalopoda_small,
                  resnep21_mpa_chondrichthyes,
                  resnep21_mpa_crustacea,
                  overwrite = TRUE)
...
x_community_mpa <- na.omit(resnep21_mpa$PesoIndividual)
range(x_community_mpa)

res_community_mpa <- fit_size_spectrum(x_community_mpa,
                                 x_min = 0.2,
                                 x_max = 1000)
res_community_mpa
plot(res_community_mpa,
     col = col_mpa)
```
...
### 2021 MPA ###
# define factors --- change according to analysis to be made ---
x_chondr_mpa <- na.omit(resnep21_mpa_chondrichthyes$PesoIndividual)
range(x_chondr_mpa)
...
res_chondr_mpa <- fit_size_spectrum(x_chondr_mpa)
...
# NTR#
# define factors --- change according to analysis to be made ---
x_ceph_all_mpa <- na.omit(resnep21_mpa_cephalopoda$PesoIndividual)

range(x_ceph_all_mpa)
# SELECTING RANGE - doing in function
# X = X[X > 2]
# X = X[X < 200]
# range(X)

res_ceph_all_mpa <- fit_size_spectrum(x_ceph_all_mpa,
                                           x_min = 2,
                                           x_max = 200)
res_ceph_all_mpa
plot(res_ceph_all_mpa,
     col = col_mpa)
```
...
# NTR#
# define factors --- change according to analysis to be made ---
x_ceph_large_mpa <- na.omit(resnep21_mpa_cephalopoda_big$PesoIndividual)

range(x_ceph_large_mpa)

res_ceph_large_mpa <- fit_size_spectrum(x_ceph_large_mpa)

res_ceph_large_mpa
plot(res_ceph_large_mpa,
     col = col_mpa)
```
...
### 2021 MPA ###
x_ceph_small_mpa <- na.omit(resnep21_mpa_cephalopoda_small$PesoIndividual)

range(x_ceph_small_mpa)

res_ceph_small_mpa <- fit_size_spectrum(x_ceph_small_mpa)

res_ceph_small_mpa
plot(res_ceph_small_mpa,
     col = col_mpa)
```
...
# 2021 MPA
resnep21_mpa_actinopterygii <- subset(resnep21_mpa_actinopterygii, Talla_mm >40)
x_actin_mpa <- na.omit(resnep21_mpa_actinopterygii$PesoIndividual)

range(x_actin_mpa)

res_actin_mpa <- fit_size_spectrum(x_actin_mpa)
...
# 2021 MPA
x_crust_mpa <- na.omit(resnep21_mpa_crustacea$PesoIndividual)

range(x_crust_mpa)

res_crust_mpa <- fit_size_spectrum(x_crust_mpa,
                                   x_min = 0.3)
