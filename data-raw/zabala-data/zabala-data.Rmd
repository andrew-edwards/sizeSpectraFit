...
HERE - above R code works, haven't run full .Rmd.
...
```{r, calcbins}
dat <- mediterranean_data[1:100, ]   # Just doing first 100

# Can see that have two 140s, so would be worth combining at this point. Maybe
# not for now though, as will be harder to compare results to Juliana's? Could
# do it and then comment out if want to compare.
dplyr::filter(dat, species == "Lepidorhombus boscii", length == 140)
...
dat_joined <- dplyr::left_join(dat,
  ...
  dplyr::mutate(weight_bin_min = alpha * (length_bin_min/10)^beta,
                ...
                biomass_bin_min = number * weight_bin_min,
                biomass_bin_max = number * weight_bin_max)
...
# weights are in grammes - have asked Juliana.
...
Just want to analyse one strata at a time. For a given data set to be fit, do a
function to combine numbers for which we have the same species and length
bin. Can actually do that early on.


Next - adapt the likelihod function presumably next. See lkelihood.R from
sizeSpectra. Need to get similar format to new sizeSpectra2 code.
