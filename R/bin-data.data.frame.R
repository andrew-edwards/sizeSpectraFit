...
##'   * indiv: data.frame with a row for each `counts_df$x` value, with columns:
...
bin_data.data.frame <- function(counts_df,
  ...
  x = counts_df$x                   # need a lot, these are measurements
  ...
  } else           # create bin_breaks based on bin_width
  {
    ...
    } else     # If not "2k"
    {
      ...
      { stop("bin_width must be 2k or a number (in quotes is okay
                         in quotes) in bin_data.data.frame().")
      }
  ...
  indiv = counts_df           # data.frame(x = x)       # data.frame with one
