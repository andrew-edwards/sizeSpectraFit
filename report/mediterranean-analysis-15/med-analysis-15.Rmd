...
```{r, loadcephdelin}
ceph_small_large_delineation <- 10    # Juliana said to use
...
```{r, crustbase}
HERE - decide what to do for classes.
...
fg_agg_appended <-
    fg_agg$crust$data %>%
    dplyr::select(-c(low_count, high_count)) %>%
    dplyr::mutate(gap = bin_min - lag(bin_max)

fg_agg_appended %>%
    tail(10)

plot(fg_agg_appended$gap)

fg_agg$crust$data %>% tail(10)
fg_crust_removed <- remove_outliers(fg_agg$crust)



```

TODO Not as clear cut as baseline, but seems sensible to remove the final one.
...
```{r, crustntrremove}
ntr_agg$crust$data %>% tail(20)
ntr_crust_removed <- remove_outliers(ntr_agg$crust,
                                     number = 4)

ntr_crust_removed %>% tail(10)
```

Suggest leaving them all in, as there's a slight jump from 110 but then still
four more values, not just one outlier. Then again, still a small proportion of
the values (since more crustaceans in general), so suggest removing the last four.
...
```{r, actinbase}
baseline_agg$actin$data %>% tail(20)
...
TODO No, have values already created., actually justNow to tease  out the $x_{\text min}$ values for this particular analysis. Safest to do
explicitly, no need for `strata_for_xmin` here:
```{r setxmin};
xmin_full <- NULL
...
xmin_cephlarge <- ceph_small_large_delineation
```

Not doing this for 11, but need it for 12, so just not running here.
Now to tease out the $x_{\text max}$ values for this particular analysis. Safest to do
explicitly, no need for `strata_for_xmax` here, set first `if()` depending on analysis
```{r setxmax}
