...
knitr::opts_chunk$set(
  ...
  comment = "",
  cache = TRUE,
  cache_path = "med-analysis-5-cache/",
  fig.path = "med-analysis-5-figs-cache/",
  fig.width = 7,
  fig.height = 9
)
  # comment = "#>",
...
rmarkdown::render("med-analysis-5.Rmd")
```
...
Using `dat_joined.rds` from `med-analysis-4.Rmd`, using the latter as
a template to shorten down the analysis, and then try different assumptions.

Here (`med-analysis-5.Rmd`) we set $x_{min}$ for each group to be the baseline value for that group, as
...
Now to tease out the $x_{min}$ values for this particular analysis. Safest to do
...
but this (and other small ones) will automatically below all $x_{min}$ we
