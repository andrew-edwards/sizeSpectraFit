...
## Full Community ntr all data (suggests removing the smallest, so doing that next)
...
```{r, fullntrdetails}
res_full_ntr$mlebins_fit$data %>% filter(bin_min < 0.5)
```
...
Can see a bit more clearly in these figures

```{r, fullntrplot2}
plot.size_spectrum_mlebin(res_full_ntr$mlebins_fit,
...
plot.size_spectrum_mlebin(res_full_ntr$mlebins_fit,
                          xlim = c(0.1, 0.5))  # note plot..._mlebin
ind <- 1
rect(xleft = res_full_ntr$mlebins_fit$data[ind, "bin_min"],
       #exaggerate bin:
     ybottom = res_full_ntr$mlebins_fit$data[ind, "low_count"] * 0.01,
     xright = res_full_ntr$mlebins_fit$data[ind, "bin_max"],
     ytop = res_full_ntr$mlebins_fit$data[ind, "high_count"],
     col = "green")
ind <- 2
rect(xleft = res_full_ntr$mlebins_fit$data[ind, "bin_min"],
     ybottom = res_full_ntr$mlebins_fit$data[ind, "low_count"] * 0.01,
