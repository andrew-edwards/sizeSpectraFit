...
length resolutions. Did some analysis (copied to here), so need to rerun all these.
...
Second piece is the definition of lengths. They are in mm. I had originally
misintepreted as integers measured to nearest mm and ones
...
Juliana: The lengths with 0.01mm accuracy are all measured crustaceans [Andy:
...
## Checking lengths -- move to elsewhere, presumably the report.

Had thought that Crustacea are the only ones measured to 0.01 mm, rest are 1 mm.

**All non-crustacea are measured to nearest 1 mm (except one measurement that
doesn't matter), as verified below.**

Looks like crustacea are not all measured to 0.01 mm though, so look at each strata
separately (below).

**Conclusion is that
crustacea in fishing grounds and NTR were measured to nearest 0.1 mm (with a
very few minor exceptions), while those for baseline were to nearest 0.01 mm.**

[should reorder into usual order when putting into report]

### Crustacea Fishing grounds:
```{r fgcrustacea2}
table(dat$length)[1:10]   # crustacea FG, looks like nearest 0.1 mm
lengths <- as.numeric(names(table(dat$length)))
...
```{r onesp}
nrow(dplyr::filter(dat, species == "Parapenaeus longirostris"))
```
...
table(crust_baseline$length)[1:100]   # looks like nearest 0.01 mm
...
setdiff(sp_to_0.01, sp_not_to_0.01)
setdiff(sp_not_to_0.01, sp_to_0.01)
dplyr::filter(crust_baseline,
              species %in% setdiff(sp_not_to_0.01, sp_to_0.01))

```
So lots of measurements to 0.01 mm, looks like a mixture of species, and the
ones that are only not to 0.01 (final tibble) is only 21 rows.
...
dplyr::filter(crust_ntr,
              !(length %in% to_0.01)) %>% summary()
sp_not_to_0.01 <- dplyr::filter(crust_ntr,
                                !(length %in% to_0.01))$species %>% unique()
...
dplyr::filter(crust_ntr,
              length %in% to_0.01)$length
```
So only two species that seem to be to 0.01, but are also measured to 0.1 it
seems.

So let's simply just assume all crustacea ntr were
to 0.1 mm (very minor approximation introduced for 3 measurements).
...
dplyr::filter(non_crust,
              length %in% to_0.1)
```
So only one non-crustacean measurement to 0.1 mm, and it's the smalles, and so
will get removed anyway most likely. So just assume all non-crustacea were
indeed measured to nearest 1 mm.
...
Note the 10 needed to convert mm lengths into weights based on length-weight
coefficients that assume cm.
...
TODO edit and summarise: Also, the results show clear distinct ranges of data.
