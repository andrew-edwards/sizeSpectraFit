...
This does some data wrangling (saving the data as `dat_joined.rds`) and then a
full analysis. Then trimming this down to use as a template for versions 5, 6,
etc. to test different settings. See READreport.txt for
details of each version (summarising the settings).
...
expect_equal(nrow(dat_with_breaks),
             nrow(dat_joined) + nrow(dat_sp_excluded))   # note dat_joined gets
                                        # changed later
...
The fits for the full community do not look great. To understand the changes we
...
saveRDS(ceph_small_large_delineation,
        "ceph_small_large_delineation.rds")
```

Was going to save `dat_joined` but with this small outlier taken out, as we
remove it both times it comes up (though in practice it would get removed when
we fix prescribed $x_{min}$ anyway):
```{r, outlier}
mediterranean_for_mlebins(dat_joined,
                          group_name = "Crustacea",
                          strata_name = "ntr") %>%
  dplyr::filter(bin_min <= 0.35)
```

But there are other small ones in the data
that get ignored when $x_{min}$ is calculated for the strate in question, so keep it in to keep everything
consistent.
```{r, savedat}
dplyr::filter(dat_joined,
              weight_bin_min < 0.35)
```

So stick with `dat_joined` as saved above.
