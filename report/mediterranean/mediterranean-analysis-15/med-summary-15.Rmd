...
knitr::opts_chunk$set(
  ...
  comment = "",
  cache = TRUE,
  cache_path = "med-summary-15-cache/",
  fig.path = "med-summary-15-figs-cache/",
...
rmarkdown::render("med-summary-15.Rmd")
```
...
This summarises the results from `med-analysis-15.Rmd`, which takes a while to run
...
given. This file can then be used to make the appendix for the desired run.
...
with assuming the same catchability between groups. Fits for all
...
## First show fits to full community to then explain why not using

```{r, fullbaselineplot, echo = FALSE, fig.cap = "Full community baseline."}
seg_col = "lightgreen"

plot(baseline_agg_list[["full"]],
     xlim_hist = c(0, 60),
     main = "Full community baseline",
     seg_col = seg_col)
```

```{r, fullfgplot, echo = FALSE, fig.cap = "Full community fishing grounds."}
plot(fg_agg_list[["full"]],
     xlim_hist = c(0, 60),
     main = "Full community FG",
     seg_col = seg_col)
```

```{r, fullntrplot, echo = FALSE, fig.cap = "Full community no-take reserve."}
plot(ntr_agg_list[["full"]],
     xlim_hist = c(0, 60),
...
# Uncommenting this and dev.off() below to just run this chunk manually to make
# a .tiff to insert into Word file
...
```{r plotsummary2, fig.width = 11, fig.height = 6, fig.cap = "As for previous Figure but restricting the y-axis (which misses the NTR results for Chondrichthyes).", echo = FALSE, eval = TRUE}
plot_multiple_exponents(res_table_agg,
                        ylim = c(-5, -1),
                        col_strata = strata_col,
                          shade_first = TRUE)
