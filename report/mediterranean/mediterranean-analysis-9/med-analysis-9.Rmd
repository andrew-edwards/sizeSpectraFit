...
knitr::opts_chunk$set(
  ...
  comment = "",
  cache = TRUE,
  cache_path = "med-analysis-9-cache/",
  fig.path = "med-analysis-9-figs-cache/",
  fig.width = 7,
  fig.height = 9
)
  # comment = "#>",
...
rmarkdown::render("med-analysis-9.Rmd")
```
...
Here (`med-analysis-9.Rmd`) we determine $x_{\text min}$ independently for all
groups (using the modal method) as for `med-analysis-4.Rmd`, and set
$x_{\text max}$ for each group to be the $x_{\text max}$ determined for the
baseline in each group `med-analysis-4.Rmd` (hadn't meant to do this, and it
will actually exclude some data maybe; good for understanding though).
...
explicitly, and then change `strata_for_xmax` for each analysis as required
