---
title: "Fitting length data with species-specific body-mass relationships - MLEbins method"
author: "Andrew Edwards"
output:
  rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Fitting length data with species-specific body-mass relationships - MLEbins method}
...
Did all this but didn't run it in one go. Have changed some functions since
doing Zabala analysis, so prob best to make sure Zabala can run first. HERE
...
essentially add uncertainty to a length. For example, a fish of length 15 cm
measured to the nearest cm has a true length in the range 14.5-15.5 cm, which
then gets converted into a body-mass range using that species specific
length-weight coefficients. See the MEPS paper for further explanation (rather
than it being repeated here, where we focus on applying the method).
...
BITER by the Instituto de Ciencias del Mar (ICM-CSIC), Spain). The full data are
available in sizeSpectra2 as simply
```{r meddata}
mediterranean_data
```
and are being analysed using sizeSpectra2 as part of a separate manuscript
(see `?mediterranean_data` for more details).
...
are close to a no-take marine reserve. So first filter to obtain just those values:
```{r fgcrustacea}
...
summary(dat)
```
The `number` column is the number of individuals of species `species` that were
measured as length `length`. Values of `number` can be non-integer because they
are standardised by dividing by trawling time TODO check; the MLEbins method can
deal with this.
...
### New function that should replace proceeding chunks

New function:
- check results agree, then change name to dat_joined
```{r convert}
dat_joined_3 <- length_bins_to_body_mass_bins(dat_all,
                                              mediterranean_length_weight_coefficients,
                                              length_data_unit = "mm")

expect_equal(dat_joined_3,
             dplyr::select(dat_joined_2,
                           -"species"))   # from below; passes!
```
...
# TODO should functionalise this; check it agrees with function
dat_joined <- dplyr::left_join(dat_all,
                               mediterranean_length_weight_coefficients,
                               by = "species") %>%

  # Lengths to weights, note the 10 needed here to convert mm lengths into
  # weights based on length-weight coefficients that assume lengths are in cm:
...
dat_joined[1:5, ] %>% a()      # a() is our helper shortcut for as.data.frame()

summary(dat_joined)
```

Doing these in length_bins_to_body_mass_bins: The data may have rows that have the same species and bins - maybe take care of
in the fitting??? TODO - yes, probably worth doing. Also sort by `weight_bin_min`:
From mediterranean_for_mlebins:
```{r duplicates}
dat_joined_2 <- dplyr::summarise(dplyr::group_by(dat_joined,
                                                 species,
                                                 weight_bin_min,
                                                 weight_bin_max),    # need to retain, not needed
                                 # for grouping
                                 bin_count = sum(number)) %>%
  dplyr::ungroup() %>%
  dplyr::arrange(weight_bin_min)

# This could go into fitting:
...
It is useful for users to retain see all the columns above, but here we just
simplify it down (and rename two columns) to what is needed for the fitting,
namely `number`, `bin_min` and `bin_max` (assumed to represent body masses);
users should do the same for their own data.
looking like:
```{r, select}
dat_use <- dplyr::select(dat_joined,
                         bin_count = number,
                         bin_min = weight_bin_min,
                         bin_max = weight_bin_max)
dat_use %>% tail()

dat_use_2 <- dplyr::select(dat_joined_2,
                         bin_count,
                         bin_min = weight_bin_min,
                         bin_max = weight_bin_max)
dat_use_2
...
Now fit the data using the MLEbins method

Those steps have been combined into a single function:
```{r, mlebins}
res <- determine_xmin_and_fit_mlebins(dat_use)
dim(dat_use)

res_2 <- determine_xmin_and_fit_mlebins(dat_use_2)
dim(dat_use_2)   # ran way quicker. So TODO absorb the grouping of same bins,
                 # and arranging, into the fitting. HERE next

```

```{r, fig}
plot(res_2)    # still not working though HERE, unlike example from med analysis
```

```{r, figlinear}
# class(res_2)
# [1] "determine_xmin_and_fit_mlebins" "list"
plot(res_2$mlebins_fit,
       style = "linear_y_axis")
```
...
need those in the `select` chunk above.
