...
value determined based on the data. You do have 
to specify the explicit method that you want used (see below). TODO
...
$x_{min}$ to use and then fit using the MLE method. 

Let's make up a mock dataset, with some uniformly distributed values between 0.1
and 10, and then a PLB distribution for values $\geq 10$:
```{r, mockdata}
set.seed(42)
mock_data <- c(runif(100, 0.1, 10),
               rPLB(1000, -2, xmin = 10))
hist(mock_data)
```
...
For a mock dataset we can just use the above `sim_vec_binned` but with a lower
count for the first bin:
```{r, mockbin}
sim_vec_binned_2 <- sim_vec_binned
sim_vec_binned_2[1, "bin_count"] <- 100
sim_vec_binned_2
```
...
note the single grey bin (the lowest bin) that is not used for the fitting.

```{r, mockbinres}
res_binned_2
```

The `res_binned_2` object is list of two objects:
...
`plot.determine_xmin_and_fit_mlebin()` is automatically used by `plot(res_binned_2)` to
give the above plot.
