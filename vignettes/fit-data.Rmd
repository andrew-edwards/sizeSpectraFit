...
think bin_data.data.frame() needs options to start at min(x) precisely, not just floor(..),
and also to truncate top bin at the max of the data. Go with: if using x values
then calc xmin and xmax exactly, use xmin to start the binning, and cut the top
bin off at xmax not the max of the generated bin. This is being done in plot_lbn_style.
...
want to test
plot_lbn_style(res_vec, x_plb = x_plb)


This works, need to tweak figure code, think done.:
