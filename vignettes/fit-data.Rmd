...
sim_vec[1:15]
length(sim_vec)
hist(sim_vec, breaks = 100)        # Quick histogram to show the long tail
```

To fit the data using the MLE method, simply:
```{r, MLE2}
res_vec <- fit_size_spectrum(sim_vec)
res_vec    # We have set it up to just print the MLE results plus only the first
           # 10 values of the original data, sim_vec.
# res_vec$x         # Print all 1000 values of the original data
length(res_vec$x)   # Confirms there are still 1000 values
```
So `res_vec` is a list that contains the MLE and confidence intervals for $b$,
plus also the original data.
Next, do a figure.
