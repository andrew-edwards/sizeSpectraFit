...
TODO After doing MLEbin below , then come back and add extra options for biomass
...
the same simulated data as used abov but only available in binned form. This is
saved in the package as a tibble as a list object (see `?sim_vec_binned` for details), for which we will fit the tibble:
```{r, MLEbin1}
...
res_binned
```
As you can see, `res_binned` is a list object that contains objects `b_mle`, the
maximum likelihood estimate of the exponent $b$, `b_conf` which gives the 95%
confidence interval of $b$, and `data` which gives the data used TODO note that
it might be different, have to check if do end up removing 0's in end
...
TODO take some text from `MLEbin_recommend` vignette, next plot matches the
figure (almost completely).
```{r, MLEbinplot}
plot(res_binned) # working on now HERE - done, need to tidy up
```

Can add options, and also just plot one or other of the panels:
```{r, MLEbinplot2}
plot(res_binned,
     x_small_ticks_labels = c(5, 50, 500))
```
...
Can delete, or maybe use some above to explain the tibble. To get your data into
the required format use `bin_data()` or whatever - see data-raw/.
