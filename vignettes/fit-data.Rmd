...
method to use based on the class of the data, as described by Table 2 in [2].

### Vector of individual measurements

This simplest method is given a vector of measurements, such as body masses or
body lengths, fit the size spectrum to calculate the maximum likelihood estimate
(the MLE method) of the size-spectrum exponent, and also provide confidence
intervals.
...
sizeSpectra2, and so we can just use that.
...
To fit the data using the MLE method, simply use [fit_size_spectrum()]:  TODO
check if that works in vignette
```{r, MLE2}
res_vec <- fit_size_spectrum(sim_vec)
res_vec
```
So `res_vec` is a list that contains the MLE and confidence intervals for $b$,
plus also the original data. When printing we have set it up to just show the
...
Now, to plot the PLB fit and the original data, simply:
```{r, MLE4}
plot(res_vec)
```

The style of plot is as recommended in Figure 6b of
[1](http://onlinelibrary.wiley.com/doi/10.1111/2041-210X.12641/full), and
includes the PLB fits using the values of `b` at the bounds of the confidence
interval for `b`.

The plot comes out this way because we have written a function
...
sim_vec_binned
```
The `2k` value says to assign bin widths that double in width.
The function returns a list of two tibbles, the first (`$bin_for_each_x` above) shows each
original value of `sim_vec`, the counts of that value (usually each count would
be 1 for accurate data, like we have in `sim_vec`), and the bin that that value
...
`?bin_data` for full details of outputs.
...
[2](https://www.int-res.com/abstracts/meps/v636/p19-33/)
