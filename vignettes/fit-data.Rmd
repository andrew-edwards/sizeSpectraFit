...
Now, to plot the PLB fit and the original data as an Individual Size
Distribution (ISD), simply do:
```{r, MLE4, fig.cap = "hello"}
plot(res_vec)
```
...
There are several styles available for easy plotting. The default is the figure
above, as
recommended in Figure 6b of
our [MEE paper](http://onlinelibrary.wiley.com/doi/10.1111/2041-210X.12641/full)
(the figure above reproduces that figure, because we used the same simulated
data here).
...
shows the normalised biomass in the top panel followed by the ISD plot shown above:
```{r, MLEbiomassplot}
plot(res_vec,
     style = "biomass")
```

In (a), the black horizontal lines indicate the normalised biomass within each
bin. We use bars, rather than the usual points (e.g. Figure 2(f) of our MEE
...
distribution, who's definition includes the $x_{min}$ and $x_{max}$ bounds (see
MEE paper equation 1), but the top bin here goes beyond $x_{max}$, even though
there are no data there.
...
To show the biomass plot and the ISD plot:
```{r, MLEbiomassisdplot}
plot(res_vec,
...
To fit the PLB distribution, we again just use the `fit_size_spectrum()` function:
```{r}
res_mlebin <- fit_size_spectrum(sim_vec_binned)
res_mlebin
```

So `res_mlebin` is a list object that contains objects `b_mle`, the
MLE of the exponent $b$ as calculated using the MLEbin method, `b_conf` which gives the 95%
confidence interval of $b$, `data` which gives the data used for the fit plus
some extra calculations (TODO explain somewwhere else), `x_min` and `x_max`
which are the MLEs of $x_{min}$ and $x_{max}$, which are simply the minimum of
the lowest bin and maximum of the maximum bin. Also, `method` tells us that the
MLEbin method was used, and the `attr(,"class")` at the end adds attributes to
the list that help our automated plotting functions.

Our default plot for results from the MLEbin method is
```{r, MLEbinplot}
plot(res_mlebin)
```

HERE - use the text from here to END, to streamline this. Decide how repetitive
need to be from the above. And whether biomass should have two plots. Maybe in
above we should have biomass and biomass_ISD styles? Cover all the options.
...
which is similar to Fig. 7 of our
[MEPS paper](https://www.int-res.com/abstracts/meps/v636/p19-33/)
but for nonbinned data. TODO check
...
This is similar to the "both" style shown above for invidual body masses, but
here we only have binned data available. in the style of
...
`r sum(res_mlebin$data$bin_count) - res_mlebin$data$bin_count[1]`
(all individuals could conceivably be 1.7 g and therefore all are $\geq$ 1.5 g)
and `r sum(res_mlebin$data$bin_count)` (all individuals in the first bin are
actually $<$ 1.5 g).
...
# Tweaking options for plots

Fix TODO's in each plotting function.

Many aspects of the plots can be adapted by changing arguments to the plotting
functions.

See the relevant helpfiles.
