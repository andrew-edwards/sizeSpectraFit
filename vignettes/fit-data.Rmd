...
TODO NEW CODE GIVES DIFFERENT CONF - previously had conf as -2.097417 -1.964317,
now have (-2.096697, -1.964697). MLEs are the same.

Need to figure
out why. Difference was on 2024-09-24, between 10:14am (when last rendered this
file) at end of day. Aha - originally had `vec_inc = 0.001`, now have `b_vec_inc
= 0.00001`.

Check what I had in sizeSpectra. Re-run first vignette code, get the same as
before (-2.097417, -1.964317). Oh, but I had
`bvec = seq(PLB.bMLE - 0.5, PLB.bMLE + 0.5, 0.00001)`  so same increment as what
I now have here. Seems like an issue in the new code then. Aha -
`calc_mle_conf()` has `vec_inc` and `b_vec_inc`; should just have the former for
generality.

Have fixed in the commit that creates this section of text. Saving then deleting
in next commit.
...
option - yes, strata = year as the default).
