...
disproportionately remove larger fish within each species group, removing 20% of
each species group.
...
for(s in 1:S){
  ...
  # Proportionately remove the large ones by making the probability of removal
  # increase as values are larger
  # ind_sizes_sum <- sum(ind_sizes^3)

  # prob_removal <- ind_sizes^3  / ind_sizes_sum

  # Sample 80% of data, weighting against high values
  #  x_values <- sample(ind_sizes,
  #                   size = length(ind_sizes) * 0.8,
  #                   replace = FALSE,
  #                   prob = 1 - prob_removal)

  # Try my original idea. Remove more large ones than small; I was thinking of
  # each one in proportion to it's index. Actually, just remove 10% of small
  # fish and 50% of large.

  num_sizes <- length(ind_sizes)   # Assume even
...
HERE - do the above, prob rerun all first. Almost there I think
...
```{r, fishedagg}
plot_aggregate(res_fished_list)
```
...
```{r combinedplotnormfunct, fig.cap = "The two aggregated plots restricted to a common range of body masses and then normalised, to properly compare them.", fig.height = 6, echo = FALSE}
plot_aggregate_fits(agg_list,
                    strata_names = strata,
                    ylim = c(10^(-4), 1))
```
HERE - need to have x_plb or something saved I think, ah from doing the plots?
See med-summary4.Rmd
...
```{r combinedplotnormlinear, fig.cap = "As previous figure but with a linear y-axis.",fig.height = 6, eval = TRUE, echo = FALSE}
plot_aggregate_fits(agg_list,
                    strata_names = strata,
                    log_axes = "x")
```
